...
#define DEFAULT_BROADSTYLE              "font-size:9px"
...
class QInputTool : public QWidget
{
    Q_OBJECT

public:
    explicit QInputTool(QWidget *parent = 0);
    ~QInputTool();

    //单例模式,保证一个程序只存在一个输入法实例对象
    static QInputTool *Instance() {
        if (!_instance) {
            _instance = new QInputTool;
        }
        return _instance;
    }

    QString str_btnPress;
    //初始化面板状态,包括字体大小
