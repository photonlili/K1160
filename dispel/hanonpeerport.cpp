...
HanonPeerPort::HanonPeerPort(QObject *parent) :
    QSerialPort(parent)
{
...
HanonPeerPort::~HanonPeerPort()
{
    close();
}

void HanonPeerPort::sendMsgConnectToC51()
{
    //if(m_conCount)
    //    return;
    m_cmdCount = 0;
...
void HanonPeerPort::sendMsgHeatPress(quint8 stage, quint8 vessel, quint16 press)
{
    m_cmdCount = 0;
...
void HanonPeerPort::sendMsgHeatStandard(quint8 stage, quint8 vessel, quint16 tempture, quint16 hold)
{
    m_cmdCount = 0;
    m_msgType = _PEER_HEAT;
...
void HanonPeerPort::sendMsgHeatRAMP(quint8 stage, quint8 vessel,
...
void HanonPeerPort::sendMsgHeatExtract(quint8 stage, quint16 tempture, quint16 hold)
{
    m_cmdCount = 0;
    m_msgType = _PEER_HEAT;
...
void HanonPeerPort::sendMsgPause()
{
    m_cmdCount = 0;
...
void HanonPeerPort::sendMsgStop()
{
    m_cmdCount = 0;
...
void HanonPeerPort::SendSerialMessage()
{
...
void HanonPeerPort::sendConnectToC51()
{
    QQTConnC51Struct t;
    QByteArray l;
    t.pack(l);
...
void HanonPeerPort::sendHeatPress()
{
    QQTHeatPressStruct t;
...
void HanonPeerPort::sendHeatStandard()
{
    QQTHeatStandardStruct t;
    t.setStage(m_heat.stage());
    t.setVessel(m_heat.vessel());
...
void HanonPeerPort::sendHeatRAMP()
{
    QQTHeatRAMPStruct t;
    t.setStage(m_heat.stage());
    t.setVessel(m_heat.vessel());
...
void HanonPeerPort::sendHeatExtract()
{
    QQTHeatExtractStruct t;
    t.setStage(m_heat.stage());
    t.setTempture(m_heat.tempture());
    t.setHold(m_heat.hold());
    QByteArray l;
...
void HanonPeerPort::readyReadData()
{
    ...
    do{
        quint16 nBlockLen = HanonPeerMessage::parseBlockSize(m_blockOnNet);
...
void HanonPeerPort::updateProgress(qint64)
{

}

void HanonPeerPort::dispatchRecvedMessage(QByteArray &blockOnNet)
{
    HanonPeerMessage qMsg;
...
void HanonPeerPort::recvCmdAck(const QByteArray &l)
{
...
void HanonPeerPort::recvCmdNAck(const QByteArray &l)
{
    pline() << "command run fail";
    // 重发
}

HanonPeerPort *HanonPeerPortInstance(QObject *parent)
{
    static HanonPeerPort* com0 = new HanonPeerPort(parent);
    return com0;
}
