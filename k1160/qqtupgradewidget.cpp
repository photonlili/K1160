#include "qqtupgradewidget.h"
#include "ui_qqtupgradewidget.h"
#include "qqtdefine.h"
#include <QTimer>
#include "qqtclient.h"
#include "qqtdefine.h"
...
void QBackupLocalThread::run()
{
    ...
    system("tar czvf ./tmp/backup.tar.gz /QQTApp/tank");
#else
...
QQTUpgradeWidget::QQTUpgradeWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::QQTUpgradeWidget)
{
    ui->setupUi(this);
    ...
    m_cli = QQTUpgradeClientInstance(this);
    m_protocol = new QQTCloudProtocol(this);
    m_cli->installProtocol(m_protocol);

    connect(m_protocol, SIGNAL(connected()), this, SLOT(logined()));
    connect(m_protocol, SIGNAL(signalCheckVersionResult()), this, SLOT(versionR()));
    connect(m_protocol, SIGNAL(signalUpdateProgress(int)), ui->widgetUpgrade, SLOT(setValue(int)));
    connect(m_protocol, SIGNAL(signalDownSucc()), this, SLOT(downOK()));
...
QQTUpgradeWidget::~QQTUpgradeWidget()
{
    delete ui;
}

void QQTUpgradeWidget::initAll()
{
...
void QQTUpgradeWidget::setText(QString text)
{
    ui->lbUpgrade->setText(text);
}

void QQTUpgradeWidget::setValue(int value)
{
    ui->widgetUpgrade->setValue(value);
}

void QQTUpgradeWidget::download()
{
    //开始下载过程
...
void QQTUpgradeWidget::logined()
{
    m_newSoftwareID = "-1";
    ui->tbVersion->clear();
    m_protocol->sendCheckVersion();
}

void QQTUpgradeWidget::versionR()
{
    QTCheckVersionResult t = m_protocol->GetVersionResult();
    if(m_newSoftwareID == "-1")
    {
        ...
        m_protocol->sendCheckNewVersion();
    }
    else
    {
        pline() << "new";
...
void QQTUpgradeWidget::on_btnDown_clicked()
{
    ui->lbUpgrade->setText(tr("Downloading..."));
    m_protocol->sendDownUpgradeFile(m_newSoftwareID, "./tmp/upgrade.tar.gz");
}

void QQTUpgradeWidget::downOK()
{
    ui->lbUpgrade->setText(tr("Download success"));
    m_upgradeT->start();
}

void QQTUpgradeWidget::restart()
{
    static int i = 6;
    if(i == 0) {
#ifdef __MIPS_LINUX__
...
void QQTUpgradeWidget::on_btnCancel_clicked()
{
...
void QQTUpgradeWidget::on_btnCheck_clicked()
{
    initAll();
}
