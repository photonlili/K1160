#include "qqtsampledatawidget.h"
#include "ui_qqtsampledatawidget.h"
#include "qqtdefine.h"
...
QQTSampleDataWidget::QQTSampleDataWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::QQTSampleDataWidget)
{
    ui->setupUi(this);
    r = QQTReportInstance(this);
...
QQTSampleDataWidget::~QQTSampleDataWidget()
{
    delete ui;
}

void QQTSampleDataWidget::initAll()
{
    ...
    this->setStyleSheet("QQTSampleDataWidget{image:url(./skin/default/bk_clean.png)}""QQTSampleDataWidget{background-color:transparent;}");
    ...
    ui->btnLeft->setStyleSheet("QPushButton{background-color:transparent;background-image: url(./skin/default/bt_pgup_normal.png)}""QPushButton:hover{background-image: url(./skin/default/bt_pgup_normal.png);}""QPushButton:pressed{background-image: url(./skin/default/bt_pgup_press.png);}");
    ui->btnLeft->setText("");
    ui->btnRight->setFlat(true);
    ui->btnRight->setFocusPolicy(Qt::NoFocus);
    ui->btnRight->setGeometry(btnX + btnXS * 1,btnY,108,44);
    ui->btnRight->setStyleSheet("QPushButton{background-color:transparent;background-image: url(./skin/default/bt_pgdn_normal.png)}""QPushButton:hover{background-image: url(./skin/default/bt_pgdn_normal.png);}""QPushButton:pressed{background-image: url(./skin/default/bt_pgdn_press.png);}");
    ui->btnRight->setText("");
    ui->btnPrint->setFlat(true);
    ui->btnPrint->setFocusPolicy(Qt::NoFocus);
    ui->btnPrint->setGeometry(btnX + btnXS * 2,btnY,108,44);
    ui->btnPrint->setStyleSheet("QPushButton{background-color:transparent;background-image: url(./skin/default/bt_printer_normal.png)}""QPushButton:hover{background-image: url(./skin/default/bt_printer_normal.png);}""QPushButton:pressed{background-image: url(./skin/default/bt_printer_press.png);}");
    ui->btnPrint->setText("");
    ui->btnDelete->setFlat(true);
    ui->btnDelete->setFocusPolicy(Qt::NoFocus);
    ui->btnDelete->setGeometry(btnX + btnXS * 3,btnY,108,44);
    ui->btnDelete->setStyleSheet("QPushButton{background-color:transparent;background-image: url(./skin/default/bt_delete_normal.png)}""QPushButton:hover{background-image: url(./skin/default/bt_delete_normal.png);}""QPushButton:pressed{background-image: url(./skin/default/bt_delete_press.png);}");
    ...
    ui->btnExport->setStyleSheet("QPushButton{background-color:transparent;background-image: url(./skin/default/bt_export_normal.png)}""QPushButton:hover{background-image: url(./skin/default/bt_export_normal.png);}""QPushButton:pressed{background-image: url(./skin/default/bt_export_press.png);}");
...
void QQTSampleDataWidget::refresh()
{
    QQTMsgBox box;
...
void QQTSampleDataWidget::exportPdf()
{
    ...
    {
        QQTTableWidget* query = new QQTTableWidget(this);
...
int QQTSampleDataWidget::printInfoDlg()
{
    QQTPrintInfoDialog dlg;
    dlg.initAll();
    int ret = dlg.exec();
...
void QQTSampleDataWidget::on_btnPrint_clicked()
{
    QFile d(QQTPluginWatcher::Instance()->printerFile());
    if(!d.exists())
    {
        QQTMsgBox::warning(this, tr("Insert Printer, please!"));
        return;
    }
    ...
    if(lid.size() <= 0)
    {
        QQTMsgBox::warning(this, tr("No selected"));
        return;
    }
    ...
    QQTMsgBox::warning(this, tr("Print success"));
    ui->btnPrint->setEnabled(true);
}


void QQTSampleDataWidget::on_btnExport_clicked()
{

#ifdef __MIPS_LINUX__
    QDir d(QQTPluginWatcher::Instance()->upanMountPath());
    if(!d.exists())
    {
        QQTMsgBox::warning(this, tr("Insert u disk, please!"));
        return;
    }
#endif
    ...
    QQTMsgBox::warning(this, tr("Export success"));
    ui->btnExport->setEnabled(true);

}

void QQTSampleDataWidget::on_btnDelete_clicked()
{
    QVector<QStringList> lid;
    ui->widgetSampleTable->selectedRows(ESampleId, lid);
    if(lid.size() <= 0)
    ...
    int ret = QQTMsgBox::question(this, tr("Delete this %1 items?").arg(lid.size()));
    if(QQTMsgBox::Rejected == ret)
        return;
    ...
    QQTMsgBox::warning(this, tr("Delete success"));
    ui->btnDelete->setEnabled(true);

    QQTCreateSysEvent("删除样品数据");
